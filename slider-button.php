  ...
  if (!function_exists('get_field') && current_user_can('activate_plugins')) {
    ...
    function sb_register_button($buttons) {
      ...
      if (is_plugin_active('advanced-custom-fields-pro/acf.php')) {
        array_push($buttons, "|", "sliderbutton");
      }
    ...
    /**
     * Ajout d'une colonne Shortcode
     */
    function sb_columns($column) {
      $column['shortcode'] = 'Shortcode';
      return $column;
    }
    add_filter('manage_posts_columns', 'sb_columns');

    /**
     * Html à afficher dans les lignes de la colonne Shortcode
     */
    function sb_rows($column, $post_id) {
      switch ($column) {
				case 'shortcode':
    ...
    add_action('manage_posts_custom_column', 'sb_rows', 10, 2);

    /**
     * Adds a box to the main column on the Post and Page edit screens.
     */
    function sb_add_meta_box() {
      add_meta_box(
        'sb_metabox_id',
        __('Shortcode', 'sb_plugin'),
        'sb_metabox_callback', 'slider', 'side'
      );
    }
    add_action('add_meta_boxes', 'sb_add_meta_box');
    ...
    /**
     * Enlève la meta boxe image mise en avant
     */
    function sb_remove_thumbnail_box() {
      remove_meta_box('postimagediv', 'slider', 'side');
    }
    add_action('do_meta_boxes', 'sb_remove_thumbnail_box');

    /*
     * Register template single
     */
		function sb_project_sheets_single($single_template) {
			global $post;
			
      if ($post->post_type == 'slider') {
				$single_template = plugin_dir_path(__FILE__) . 'single-slider.php';
      }
      return $single_template;
    }
		add_filter('single_template', 'sb_project_sheets_single');
